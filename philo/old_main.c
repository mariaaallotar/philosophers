...
int	create_thread(t_philo *philo_info_copy, int i, pthread_t *threads, t_philo **philo_infos)
{
	t_philo	*philo_info;
...
pthread_t	*create_threads(t_philo *philo_info, t_philo **philo_infos)
{
	pthread_t	*threads;
	int			i;
	...
	while (i < philo_info->num_of_philos)
	{
		if (create_thread(philo_info, i, threads, philo_infos) == -1)
			return (NULL);
		i++;
	}
	return (threads);
...
int	main(int argc, char *argv[])
{
	...
	if (init_philo_struct(&philo_info, argv, &somebody_died) == -1)
		return (1);
	philo_infos = malloc(philo_info.num_of_philos * sizeof(t_philo *));
	if (philo_infos == NULL)
	{
		//free_all();
		return (1);
	}
	threads = create_threads(&philo_info, philo_infos);
	if (threads == NULL)
		return (1);
	while (1)
	{
		...
		while (i < philo_info.num_of_philos)
		{
			//printf("Looking if %i died of hunger\n", philo_infos[i]->philo_num);
			if (died_of_hunger(philo_infos[i]))
				break ;
			i++;
		}
		...
		{
			detach_threads(threads, philo_info.num_of_philos);
			printf("%lu %i died\n", get_current_time_milliseconds(&philo_info), *philo_info.somebody_died);
			break ;
		}
		usleep(100000);
	}
